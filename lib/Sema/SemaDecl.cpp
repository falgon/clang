...
//@@
//@@@
    if (CurContext->allowUnresolvedIds() && allowUnknownType) {
      if (SS && SS->isNotEmpty()) {
        NestedNameSpecifierLoc QualifierLoc = SS->getWithLocInContext(Context);
        QualType T = CheckTypenameType(ETK_None, SourceLocation(), QualifierLoc,
                                       II, NameLoc);
        ...
        return ActOnUnknownType(II, NameLoc);
    }
//@@
    if (CorrectedII) {
...
ParsedType Sema::ActOnUnknownType(const IdentifierInfo &II,
                                                SourceLocation NameLoc) {
  // Build a fake DependentNameType with no nested name specifier.
...
//@@
    ...
    Diag(IILoc, diag::err_typename_missing_meta_generated)
      << II->getName()
      << SourceRange(IILoc, II->getFETokenInfo<Expr>()->getLocEnd())
      << FixItHint::CreateInsertion(IILoc, "typename ");
    SuggestedType = ActOnTypenameType(S, SourceLocation(),
                                      CXXScopeSpec(), *II, IILoc).get();
	}
	else
//@@
...
//@@
	//@@@
	//Within QQs we treat any unknown symbol as an Id
	//This means that any unknown type should be specified as typename T
...
bool Sema::diagnoseQualifiedDeclaration(CXXScopeSpec &SS, DeclContext *DC,
  ...
  // declaration.
  if (!Cur->Encloses(DC) /*@@*/ && !CurContext->allowUnresolvedIds() /*@@*/) {
    if (Cur->isRecord())
...
Sema::ActOnFunctionDeclarator(Scope *S, Declarator &D, DeclContext *DC,
  ...
  if (getLangOpts().CPlusPlus) {
    ...
    if (isVirtual && !NewFD->isInvalidDecl()) {
      if (!isVirtualOkay /*@@*/ && !S->getQuasiQuotesParent() /*@@*/) {
        Diag(D.getDeclSpec().getVirtualSpecLoc(),
             diag::err_virtual_non_function);
      } else if (!CurContext->isRecord() /*@@*/ && !S->getQuasiQuotesParent() /*@@*/) {
  ...
  if (!getLangOpts().CPlusPlus) {
    ...
    if (isFunctionTemplateSpecialization && (isFriend /*@@*/ || isa<QuasiQuotesDecl>(DC) /*@@*/) &&
        (NewFD->getType()->isDependentType() || DC->isDependentContext() ||
         TemplateSpecializationType::anyDependentTemplateArguments(
            TemplateArgs.getArgumentArray(), TemplateArgs.size(),
            InstantiationDependent))) {
      assert((HasExplicitTemplateArgs /*@@*/ || isa<QuasiQuotesDecl>(DC) /*@@*/) &&
    ...
    if (NewFD->isInvalidDecl()) {
      ...
      if (D.getCXXScopeSpec().isSet()) {
        ...
        // to something.
      } else if (isFriend && /*@@*/ isa<CXXRecordDecl>(CurContext) && /*@@*/
