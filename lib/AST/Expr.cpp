...
//@@
void QuotedElements::clear() {
	switch(type) {
		case AST_Expression:											value.expr = nullptr;	break;
		case AST_StatementOrDeclarationList:	delete[]value.stmts;	value.stmts = nullptr;	break;
		case AST_ParameterDeclaration:			delete	value.params;	value.params= nullptr;	break;
		case AST_DeclContext:											value.dc	= nullptr;	break;
		case AST_TypeInfo:												value.type	= nullptr;	break;
		default:	break;
	}
	size = 0;
	type = AST_Invalid;
...
void QuotedElements::setStmts(const StmtVector& stmts) {
	clear();
	type = AST_StatementOrDeclarationList;
	size = stmts.size();
	value.stmts = new Stmt*[size + 1];
	unsigned i = 0;
	for (Stmt *S : stmts)
		value.stmts[i++] = S;
	value.stmts[i] = nullptr;
}

QuotedElements::child_range QuotedElements::children() {
	switch(type) {
		case AST_Expression:					return child_range((Stmt**)&value.expr, (Stmt**)&value.expr + 1);
		case AST_StatementOrDeclarationList:	return child_range(value.stmts, value.stmts + size);
		default:								return child_range(child_iterator(), child_iterator());
	}
}
//@@
...
StringRef UnaryOperator::getOpcodeStr(Opcode Op) {
  switch (Op) {
//@@
  case UO_Escape: return ".~";
  case UO_Inline: return ".!";
//@@
...
//@@
  case QuasiQuoteExprClass: {
    //@@TODO: should get and check all escapes
    for (const Stmt *SubStmt : cast<QuasiQuoteExpr>(this)->getEscapes())
      if (SubStmt &&
