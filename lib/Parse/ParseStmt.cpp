...
/// [MetaC++]   '.&' statement
///
StmtResult Parser::ParseExecuteStatement() {
  assert(Tok.is(tok::periodamp) && "Not an execute stmt!");
  SourceLocation ExecuteLoc = ConsumeToken();  // eat the '.&'.
  StmtResult Stmt(ParseStatement());
  return Actions.ActOnExecuteStmt(ExecuteLoc, PrevTokLocation, Stmt.get());
}
//@@
...
StmtResult Parser::ParseCXXTryBlockCommon(SourceLocation TryLoc, bool FnTry) {
  ...
  if ((Tok.is(tok::identifier) &&
    ...
    if (!Tok.isOneOf(tok::kw_catch, tok::periodtilde, tok::periodexclaim))	//@@was: Tok.isNot(tok::kw_catch)
      return StmtError(Diag(Tok, diag::err_expected_catch));
    while (Tok.isOneOf(tok::kw_catch, tok::periodtilde, tok::periodexclaim)) {	//@@was: Tok.is(tok::kw_catch)
//@@
      //@@was: StmtResult Handler(ParseCXXCatchBlock(FnTry));
      StmtResult Handler(Tok.is(tok::kw_catch) ? ParseCXXCatchBlock(FnTry) : ParseExprStatement());
//@@
      if (!Handler.isInvalid())
