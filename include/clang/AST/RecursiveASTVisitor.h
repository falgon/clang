...
// All unary operators.
#define UNARYOP_LIST()                                                         \
/*@@*/ OPERATOR(Escape) OPERATOR(Inline) /*@@*/                                \
...
//@@
DEF_TRAVERSE_DECL(EscapeDecl, { TRY_TO(TraverseStmt(D->getExpr())); })

DEF_TRAVERSE_DECL(InlineDecl, { TRY_TO(TraverseStmt(D->getExpr())); })

DEF_TRAVERSE_DECL(ExecuteDecl, {
...
DEF_TRAVERSE_DECL(DefDecl, {})
DEF_TRAVERSE_DECL(PreprocessorDirectiveDecl, {})
DEF_TRAVERSE_DECL(QuasiQuotesDecl, {})
//@@
...
//@@
DEF_TRAVERSE_STMT(QuasiQuoteExpr, {
  if (QuotedElements *Elems = S->getElements())
    if (Elems->isParamDeclaration())
      for (auto& P : *Elems->getParams()) {
	    TRY_TO(TraverseDecl(P.Param));
	  }
    else if (Elems->isDeclContext())
	  TRY_TO(TraverseDeclContextHelper(Elems->getDeclContext()));
	else if (Elems->isTypeInfo())
      TRY_TO(TraverseTypeLoc(Elems->getTypeInfo()->getTypeLoc()));
})
//@@
